...
impl MacAddr {
    /// Parse a string into an MacAddr object.
    ...
    pub fn from_bytes_unchecked(src: &[u8]) -> MacAddr {
        let mut bytes = [0u8; MAC_ADDR_LEN];
        let _ = &bytes[..].copy_from_slice(src);
    ...
    pub fn from_bytes(src: &[u8]) -> Result<MacAddr, MacError> {
        if src.len() != MAC_ADDR_LEN {
            return Err(MacError::MacLengthError(src.len()));
        }
        Ok(MacAddr::from_bytes_unchecked(src))
    }

    /// Get raw bytes of the MacAddr object.
    #[inline]

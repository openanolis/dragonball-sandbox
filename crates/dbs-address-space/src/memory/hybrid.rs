...
mod tests {
    ...
    fn test_write_and_read_on_mmap_region() {
        ...
        assert_eq!(
            format!(
                "{:?}",
                guest_region
                    .write(&mut buf_to_write, invalid_addr)
                    .err()
                    .unwrap()
            ),
    ...
    fn test_read_from_and_write_to_on_mmap_region() {
        ...
        let write_addr = MemoryRegionAddress(0x400);
        let size_of_file = 8;
        // TODO: Content of file not varified, due to the failure of writting content into TempFile generated file.
    ...
    #[test]
    fn test_bitmap() {
        // TODO: Need futher and detailed test on bitmap object.
        let start_addr = GuestAddress(0x0);
        let mmap_reg =
            GuestRegionMmap::new(MmapRegion::<()>::new(0x800).unwrap(), start_addr).unwrap();
        let guest_mmap_region = GuestRegionHybrid::from_mmap_region(mmap_reg);
    ...
    // TODO: The following function are not yet implemented:
    // - 'fn file_offset()'
    // - 'unsafe fn as_slice()'

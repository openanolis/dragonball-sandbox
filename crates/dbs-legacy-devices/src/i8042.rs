...
pub enum Error {
    #[error("Could not clone CPU reset eventfd: {0}.")]
...
impl I8042Device {
    ...
    /// Triggers a keyboard interrupt event.
...
impl DeviceIoMut for I8042Device {
...
mod tests {
    ...
    impl I8042Device {
        fn write_simple(&mut self, addr: u16, data: &[u8]) {
            self.pio_write(PioAddress(0), PioAddress(addr), data);
        }

        fn read_simple(&mut self, addr: u16, data: &mut [u8]) {
            self.pio_read(PioAddress(0), PioAddress(addr), data);
        }
    }
    ...
    fn test_i8042_read_write_and_event() {
        ...
        let mut data = [1, 2];
        i8042.read_simple(0, &mut data);
        assert_eq!(data, [1, 2]);
        i8042.read_simple(1, &mut data);
        assert_eq!(data, [1, 2]);
        ...
        i8042.write_simple(OFS_STATUS, &data);
    ...
    fn test_i8042_commands() {
        ...
        data[0] = 0x52;
        i8042.write_simple(OFS_DATA, &data);
        data[0] = CMD_READ_CTR;
        i8042.write_simple(OFS_STATUS, &data);
        assert_ne!(i8042.status & SB_OUT_DATA_AVAIL, 0);
        i8042.read_simple(OFS_DATA, &mut data);
    ...
    fn test_i8042_kbd() {
        ...
        fn expect_key(i8042: &mut I8042Device, key: u16) {
            ...
            i8042.read_simple(OFS_STATUS, &mut data);

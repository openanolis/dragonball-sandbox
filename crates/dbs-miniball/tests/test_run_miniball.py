...
"""Run the Miniball and shut it down through a command on the serial."""
import fcntl
import json
import os
import subprocess
...
def test_expect_string_timeout():
    ...
    with pytest.raises(TimeoutError) as e:
        # Let's expect a string that cannot show up as part of booting the Miniball.
        expected_string = "Goodbye, world, from the Miniball!"
...
def test_miniball(kernel):
    """Start the Miniball and verify that it works."""
    ...
    expected_string = "Hello, world, from the Miniball!"
    expect_string(vmm_process, expected_string)

    shutdown(vmm_process)

@pytest.mark.parametrize("kernel", KERNELS_INITRAMFS)
def test_miniball_with_deault_cmdline(kernel):
    """Start the Miniball with default cmdline and verify that it works."""
...
def test_miniball_with_disk(kernel, disk):
    """Start the Miniball with a block device and verify that it works."""
...
@pytest.mark.parametrize("kernel", KERNELS_INITRAMFS)
def test_miniball_num_vcpus(kernel):
    """Start the Miniball and verify the number of vCPUs."""
...
@pytest.mark.parametrize("kernel", KERNELS_INITRAMFS)
def test_miniball_mem(kernel):
    """Start the Miniball and verify the amount of guest memory."""

...
where
    ...
    T: Default + FamStruct + Versionize,
{
    #[inline]
    fn serialize<W: std::io::Write>(
        &self,
    ...
    fn deserialize<R: std::io::Read>(
        ...
        let header = T::deserialize(&mut reader, version_map)
            .map_err(|err| VersionizeError::Deserialize(format!("{:?}", err)))?;
        let entries: Vec<<T as FamStruct>::Entry> = Vec::deserialize(&mut reader, version_map)
            .map_err(|err| VersionizeError::Deserialize(format!("{:?}", err)))?;
...
mod tests {
    ...
    fn test_bincode_deserialize_from_versionize() {
        ...
        Versionize::serialize(&test_struct, &mut snapshot_mem.as_mut_slice(), &vm).unwrap();

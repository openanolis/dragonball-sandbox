...
use crate::helpers::{collect_version, parse_version};
...
impl Descriptor for StructDescriptor {
    fn generate_serializer(&self) -> proc_macro2::TokenStream {
        ...
        //let last_version = self.versions.last_key_value().map_or(;
    ...
    fn generate_deserializer(&self) -> proc_macro2::TokenStream {
        ...
        quote! {
            ...
            //let cur = parse_version(env!("CARGO_PKG_VERSION"));
            // TODO: Unsupport src.minor less then cur.minor - 2
            let src = version_map.get_crate_version(env!("CARGO_PKG_NAME")).expect("get crate version");
            match (src.minor, src.patch) {
